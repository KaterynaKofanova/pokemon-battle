...
const EntryForm = ({ setPokeOne, setPokeTwo, setTypesOne, setTypesTwo }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!PokemonNames.includes(nameOne)) {
      setMessage(
        `${nameOne} is not a valid pokemon name! Try a different name.`
      );
      setLoading(false);
    } else if (!PokemonNames.includes(nameTwo)) {
      setMessage(
        `${nameTwo} is not a valid pokemon name! Try a different name.`
      );
      setLoading(false);
    } else {
      setMessage(null);
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <p>
            How about{" "}
            <span
              onClick={() => setNameOne(randomPokeOne)}
        ...
        <div>
          ...
          <p>
            How about{" "}
            <span
              onClick={() => setNameTwo(randomPokeTwo)}
              className="RandomPoke"
            >
              {randomPokeTwo}
            </span>
